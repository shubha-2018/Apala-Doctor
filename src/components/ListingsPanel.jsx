...
const filters = ["All", "Top Rated", "Open Now", "Verified"];
...
export default function ListingsPanel({
  ...
  }, [queryFiltered, selectedFilter]);

  const items = filterApplied.slice(0, MAX_RESULTS);
  ...
  const subtitle = [
    ...
    `${filterApplied.length} result${
      filterApplied.length !== 1 ? "s" : ""
    } found`,
    filterApplied.length > MAX_RESULTS &&
      `(showing ${MAX_RESULTS})`,
  ]
    .filter(Boolean)
    .join(" • ");
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 bg-white flex flex-col overflow-hidden">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-100 z-10">
          ...
          {/* Filters */}
          <div className="px-4 sm:px-6 pb-3 overflow-x-auto scrollbar-hide max-w-7xl mx-auto">
            <div className="flex gap-2 min-w-max">
              {filters.map((filter) => (
                <button
                  key={filter}
                  onClick={() => setSelectedFilter(filter)}
                  className={`whitespace-nowrap px-3 sm:px-4 py-2 rounded-full text-xs font-semibold transition-all ${
